...
# ----------------- Utility Functions -----------------
def apply_hdr_relighting(albedo, diffuse, specular, light_direction, normals):
    albedo = albedo / 255.0  # Normalize albedo

    # Calculate the dot product between light direction and surface normal
    light_dot = np.sum(normals * light_direction, axis=2, keepdims=True)  # Dot product with normal
    light_dot = np.clip(light_dot, 0, 1)  # Ensure the dot product is within valid range (0 to 1)

    # Apply lighting model (diffuse and specular)
    diffuse_shading = diffuse * light_dot  # Diffuse shading based on light orientation
    result = albedo * diffuse_shading + specular  # Combine diffuse and specular components

    result = np.clip(result, 0, 1)  # Clamp the result to [0, 1]
...
def update_image():
    ...
    # Get light direction from sliders
    lx = light_dx.get() / 100.0
    ly = light_dy.get() / 100.0
    lz = light_dz.get() / 100.0
    ...
    relit_face = apply_hdr_relighting(albedo_img.copy(), mod_diffuse, mod_specular, light_direction, normal_map)
...
# Load normal map (assuming it is available as a normal map image)
normal_map = cv2.imread("normal_image.png")  # Load the normal map
normal_map = cv2.cvtColor(normal_map, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0

# Normalize the normal map to be in the range [-1, 1]
normal_map = (normal_map - 0.5) * 2  # Convert from [0, 1] to [-1, 1]
...
normal_map = cv2.resize(normal_map, (albedo_img.shape[1], albedo_img.shape[0]))
...
light_dx = Scale(control_frame, from_=-100, to=100, orient=HORIZONTAL, label="Light X", command=lambda _: update_image())
light_dx.set(0)
light_dx.pack(fill="x")

light_dy = Scale(control_frame, from_=-100, to=100, orient=HORIZONTAL, label="Light Y", command=lambda _: update_image())
light_dy.set(0)
light_dy.pack(fill="x")

light_dz = Scale(control_frame, from_=-100, to=100, orient=HORIZONTAL, label="Light Z", command=lambda _: update_image())
light_dz.set(1)
light_dz.pack(fill="x")
...
# Initial render
relit_initial = apply_hdr_relighting(albedo_img.copy(), diffuse_img, specular_img, np.array([0, 0, 1]), normal_map)
blended = original_img.copy()
