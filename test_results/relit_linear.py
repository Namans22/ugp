...
def relight(albedo, normals, light_x, light_y, light_z, light_color):
    ...
    reflect_dir = -(2 * dot_nl * normals - light_dir)  #2 * dot_nl * normals - light_dir
...
def update_image():
    ...
    img = Image.fromarray((result_srgb * 255).astype(np.uint8))
...
albedo_img = srgb_to_linear(albedo_srgb)  # Now in linear RGB
...
root = tk.Tk()
root.title("Phong Relighting with Colored Light")
...
result_img = relight(albedo_img.copy(), normal_img.copy(), 0.5, 0.5, 1.0, initial_light_color)
result_img = linear_to_srgb(result_img)
img = Image.fromarray((result_img * 255).astype(np.uint8))
imgtk = ImageTk.PhotoImage(image=img)
panel.config(image=imgtk)
panel.image = imgtk
